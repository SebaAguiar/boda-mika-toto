...
import { getUserByName, updateAttendance } from '@/app/serrvices/users';
...
const ConfirmModal: React.FC = (): React.JSX.Element => {
  ...
  const handleSelectChange = (event: React.ChangeEvent<HTMLSelectElement>, user: any) => {
  ...
  const handleButtonClick = async (user: any) => {
    const value = selectedValues[user.id];
    const attending = value === "Voy";

    // Envía la solicitud al servidor
		await updateAttendance({id: user.xata_id, attending});
      ...
      >
        ...
        >
          ...
          <form
            ...
            className="w-full h-max rounded-full mx-auto flex items-start justify-center"
          >
            {formInputs.map(
              (input): React.JSX.Element => (
                <div key={input.name} className="relative z-0 w-3/6 group">
                  <input
                    type={input.type}
                    name={input.name}
                    id={input.name}
            ...
            <button
              type="submit"
              className="text-white bg-[#83968d] hover:bg-[#83968d] focus:ring-4 focus:outline-none focus:ring-gray-300 font-medium rounded-lg text-sm w-auto px-5 py-2.5 text-center dark:bg-[#83968d] dark:hover:bg-[#83968d] dark:focus:ring-[#83968d] mx-5"
            >
              Buscar
            </button>
          </form>
            ...
            {foundUser.map((user: any) => (
              <div key={user.name} className='w-full h-max flex flex-col items-center justify-center m-2'>
                ...
                <select
                  ...
                  defaultValue={
										user.status === null ? '-' : user.status ? 'Voy' : 'No voy'
                  }
                >
                  {user.status === null && <option value="-">-</option>}
